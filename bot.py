    ...
    async def index(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        try:
            ...
            while current_message_id > 1:
                ...
                logger.info(f"Saving last indexed message_id {current_message_id} for chat {chat_id}")
                ...
                await asyncio.sleep(5)  # 5 seconds to prevent pool timeout
            ...
            logger.error(f"Database method error during indexing: {str(e)}")
            async with self.semaphore:
                await context.bot.send_message(
                    chat_id=chat_id,
                    text=f"Database error during indexing: {str(e)}. Please ensure the database is correctly configured."
                )
